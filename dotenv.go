...
Parsing rules:
	...
	Variables should be stored as Key-Value pairs separated by "=" sign.
	Whitespaces around key and value are trimmed.
	Lines starting with "#" sign are omitted.
	Blank lines are omitted.
	Environment variables that are already exported are not getting overwritten by those read from .env file.
